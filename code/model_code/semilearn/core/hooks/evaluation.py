...
class EvaluationHook(Hook):
    ...
    def after_train_step(self, algorithm):
        if self.every_n_iters(algorithm, algorithm.num_eval_iter) or self.is_last_iter(algorithm):
            ...
            if '_vis' in algorithm.args.load_path:
                fw=open(algorithm.args.algorithm+'_test.txt','a')
                fw.write('{} {}'.format(algorithm.it,eval_dict['eval/top-1-acc'])+'\n')
            algorithm.log_dict.update(eval_dict)

            # update best metrics
            if algorithm.log_dict['eval/top-1-acc'] > algorithm.best_eval_acc:
            # if algorithm.log_dict['eval/F1'] > algorithm.best_eval_acc:
