...
def get_json_dset(args, alg='fixmatch', dataset='acmIb', num_labels=40, num_classes=20, data_dir='./data', index=None, include_lb_to_ulb=True, onehot=False):
        ...
        if ('mine' not in alg and 'remix' not in alg and 'ver' in dataset) or ('uda' in alg):
            with open(os.path.join(json_dir, 'train_unlabel.json'), 'r') as json_data:
                train_unlabel_data = json.load(json_data)
                train_unlabel_sen_list = []
                train_unlabel_label_list = []
                for idx in train_unlabel_data:
                    train_unlabel_sen_list.append((train_unlabel_data[idx]['ori'],train_unlabel_data[idx]['aug_0']))
                    train_unlabel_label_list.append(0)
            with open(os.path.join(json_dir, 'train_label.json'), 'r') as json_data:
                train_data = json.load(json_data)
                train_sen_list = []
                train_label_list = []
                for idx in train_data:
                    train_sen_list.append([train_data[idx]['ori']])
                    train_label_list.append(int(train_data[idx]['label']))
        elif ('mine' in alg or 'remix' in alg) and 'ver' in dataset:
            with open(os.path.join(json_dir, 'train_unlabel.json'), 'r') as json_data:
                train_unlabel_data = json.load(json_data)
                train_unlabel_sen_list = []
                train_unlabel_label_list = []
        ...
        #     lb_dset = BasicDataset(alg, train_sen_list[:num_labels], train_label_list[:num_labels], num_classes, False,onehot)
        #     return lb_dset, None, dev_dset, test_dset
        if 'ver' in dataset or dataset=='pubmedqa':
            lb_sen_list, lb_label_list, ulb_sen_list, ulb_label_list=train_sen_list,train_label_list,train_unlabel_sen_list,train_unlabel_label_list
        else:
