...
class ABCNet(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        if not isinstance(results_dict, dict):
            results_dict['logits_aux'] = self.aux_classifier(results_dict)
        else:
            results_dict['logits_aux'] = self.aux_classifier(results_dict['feat'])#8,768
        return results_dict
...
class ABC(ImbAlgorithmBase):
    ...
    def compute_abc_loss(self, logits_x_lb, y_lb, logits_x_ulb_w, logits_x_ulb_s):
        ...
        abc_lb_loss = (self.ce_loss(logits_x_lb, y_lb, reduction='none') ).mean()
